...
int32_t main()
{
    ...
    for (auto tc = 1; tc <= t; tc++)
    {
        ...
        //     for (int i = 0; i < 26; i++)
        //     {
        //         if(startIdx[i].size() == 0) continue;
        //         cout << (char)('a' + i) << " : ";
        //         for (auto e : startIdx[i])
        //             cout << e << ' ';
        //         cout << endl;
        //     }
        // }
        ...
        for (auto c : startIdx)
        {
            for (int i = 0; i < c.size() && !valid; i++)
            {
                for (int j = i + 1; j < c.size() && !valid; j++)
                {
                    ...
                    for (int k = 0; !valid && (s[astart + k] == s[cstart + k]) && (astart + k < cstart); k++)
                    {
                        ...
                        for (int l = 0; l < 5; l++)
                        {
                            avow[l] = vowelsInRange[ae][l] - vowelsInRange[as - 1][l];
                            bvow[l] = vowelsInRange[be][l] - vowelsInRange[bs - 1][l];
                            cvow[l] = vowelsInRange[ce][l] - vowelsInRange[cs - 1][l];
                        }
                        int avowFreq = 0, bvowFreq = 0, cvowFreq = 0;
                        for (int l = 0; l < 5; l++)
                        {
                            avowFreq += avow[l];
                            bvowFreq += bvow[l];
                        ...
                        //     {
                        //         cout << as << ' ' << ae << " | ";
                        //         cout << avowFreq << ' ' << bvowFreq << ' ' << cvowFreq << endl;
                        //     }
                        // }

                        if (avowFreq >= 2 && bvowFreq >= 1 && cvowFreq >= 2)
                        {
                            // cout << "\n{" << as << ' ' << ae << "}\n";
                            // cout << "{" << bs << ' ' << be << "}\n";
                            // cout << "{" << cs << ' ' << ce << "}\n";
